...
# additive lmer +
#fitA = lmer( formula = D1S1T ~ param +(1|SubID), data = my_data)
#print(fitA)
#summary(fitA)

#post hoc of lmer
#emmeans(fitA, list(pairwise ~param), adjust = "tukey")
#emm_options(lmerTest.limit = 763, lmer.df = "satterthwaite")
...
# Create the ggboxplot with reordered x-axis labels and modified boxplot outline
...
my_data <- my_data %>% 
  filter(!(elapsed_timeEXP2 < lower_bound | elapsed_timeEXP2 > upper_bound))
...
ggplot(my_data, aes(x = param, y = normalized_elapsed_timeEXP2)) +
  ...
  ylim(0, 3.5) +  # Adjust the limits of the y-axis if needed
  labs(title = "Violin Plot of Normalized Elapsed Time ES Phase", 
       x = "Value Difference Levels", y = "Normalized Elapsed Time ES") +  
  theme_gray()
```
...
# Create the plot with the reordered levels
ggplot(my_data, aes(x = param, y = elapsedTimeEXP)) + 
  geom_boxplot(color = "darkorchid", fill = "#69b3a2", size = 0.8) + 
  ylim(0, 3.5) + 
  labs(title = "Boxplot of Elapsed Time LE Phase", 
       x = "Value Difference Levels", 
       y = "Elapsed Time ES") + 
  theme_gray()
```
...
# Create the plot with the reordered levels
ggplot(my_data_pie, aes(x = selected_imageEXP2, y = elapsed_timeEXP2)) + 
  geom_boxplot(color = "darkorchid", fill = "#d7745b", size = 0.8) + 
  ylim(0, 1.5) + 
  labs(title = "Boxplot of Elapsed Time by Selected Pie Images", 
       x = "Selected Pie Images", 
       y = "Elapsed Time") + 
  theme_gray()
```
...
ggplot(my_data_pie, aes(x = selected_imageEXP2, y = elapsed_timeEXP2, fill = selected_imageEXP2)) +
  geom_violin(trim = FALSE) +
  labs(title = "Violin Plot of Elapsed Time by Pie Images",
       x = "Selected Pie Images",
       y = "Elapsed Time") + theme_minimal()
...
# Extract data for correct and wrong decisions, dropping NA values
...
data_summary <- data.frame(
  Labels = c("Correct Decisions", "Wrong Decisions"),
  Mean = c(mean_correct, mean_wrong),
  SEM = c(sem_correct, sem_wrong)
)
...
ggplot(data_summary, aes(x = Labels, y = Mean, fill = Labels)) +
  ...
  labs(title = "Mean Reaction Time for Correct and Wrong Decisions Across All Participants",
